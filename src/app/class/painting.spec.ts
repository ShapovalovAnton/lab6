...
describe(`Painting testing`,()=>{
    let statue:Painting;
    beforeEach(()=>
    statue=new Painting(1, 100, "Зевс", 1800, 'Англія', 'Джек Лондон', 'Античність'));
    it("Створення екземпляру класу Картина",()=>{
        expect(statue).toBeTruthy();
    });
    it("Створення екземпляру класу Картина з від'ємним id",()=>{
        expect(()=> new Painting(-1, 100, "Зевс", 1800, 'Англія', 'Джек Лондон', 'Античність')).toThrow(new Error('id<0'));
    });
    it("Створення екземпляру класу Картина з від'ємною ціною price",()=>{
        expect(()=> new Painting(1, -100, "Зевс", 1800, 'Англія', 'Джек Лондон', 'Античність')).toThrow(new Error('price<=0'));
    });
    it("Створення екземпляру класу Картина з від'ємною роком year",()=>{
        expect(()=> new Painting(1, 100, "Зевс", -1800, 'Англія', 'Джек Лондон', 'Античність')).toThrow(new Error('year<0'));
    });
