...
describe(`Statue testing`,()=>{
    let statue:Statue;
    beforeEach(()=>
    statue=new Statue(1, 100, "Зевс", 1800, 'Англія', 'Джек Лондон', 'Гіпс'));
    it("Створення екземпляру класу Монета",()=>{
        expect(statue).toBeTruthy();
    });
    it("Створення екземпляру класу Статуя з від'ємним id",()=>{
        expect(()=> new Statue(-1, 100, "Зевс", 1800, 'Англія', 'Джек Лондон', 'Гіпс')).toThrow(new Error('id<0'));
    });
    it("Створення екземпляру класу Статуя з від'ємною ціною price",()=>{
        expect(()=> new Statue(1, -100, "Зевс", 1800, 'Англія', 'Джек Лондон', 'Гіпс')).toThrow(new Error('price<=0'));
    });
    it("Створення екземпляру класу Статуя з від'ємною роком year",()=>{
        expect(()=> new Statue(1, 100, "Зевс", -1800, 'Англія', 'Джек Лондон', 'Гіпс')).toThrow(new Error('year<0'));
    });
